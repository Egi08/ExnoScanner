...
output_dir="$HOME/exnoscan/output"
...
prepare_output_files() {
    ...
    for file in xss_vul.txt open_redirect_vul.txt lfi_vul.txt multiple_vulnerabilities.txt subdomains.txt alivesub.txt final.txt whatweb.txt katana_endpoints.txt subzy_results.txt; do
        sudo > "$output_dir/$file"  # Truncate (empty) the files
    done
}
...
run_whatweb_scan() {
    ...
    sudo whatweb -a 3 "$website_url" | tee "$output_dir/whatweb.txt"
}
...
find_subdomains_and_endpoints() {
    ...
    sudo subfinder -d "$website_input" -all -recursive > "$output_dir/subdomains.txt"
    
    echo -e "${colors[yellow]}[+] Filtering alive subdomains...${colors[reset]}"
    cat "$output_dir/subdomains.txt" | sudo httpx -silent > "$output_dir/alivesub.txt"
    
    echo -e "${colors[yellow]}[+] Checking for subdomain takeover vulnerabilities using Subzy...${colors[reset]}"
    sudo subzy run --targets "$output_dir/subdomains.txt" | tee "$output_dir/subzy_results.txt"
    ...
    while IFS= read -r subdomain; do
        ...
        sudo katana -u "$subdomain" -d 5 -silent -o "$output_dir/katana_endpoints.txt"  # Crawl subdomains with Katana
    done < "$output_dir/alivesub.txt"

    # Run ParamSpider on alive subdomains to gather potential parameters
    paramspider_results_dir="$HOME/exnoscan/results"
    echo -e "${colors[blue]}[+] Clearing previous ParamSpider results in $paramspider_results_dir...${colors[reset]}"
    sudo rm -rf "$paramspider_results_dir"
    sudo mkdir -p "$paramspider_results_dir"
    sudo paramspider -l "$output_dir/alivesub.txt"
...
find_sqli_vulnerabilities() {
    ...
    sudo rm -rf "$sqlmap_output_dir"
    sudo mkdir -p "$sqlmap_output_dir"
    ...
    if [[ -f "$output_dir/final.txt" && -s "$output_dir/final.txt" ]]; then
        ...
        while IFS= read -r url; do
            ...
            sqlmap_output=$(sudo sqlmap -u "$url" --batch --smart --ignore-redirects --level=5 --risk=3 --random-agent --technique=BEUT --output-dir="$sqlmap_output_dir" -v 3 | tee /dev/tty)
...
run_lfi_scan() {
    ...
    lfi_payloads="$HOME/exnoscan/lfi.txt"
    
    if [[ -f "$lfi_payloads" ]]; then
        while IFS= read -r url; do
            echo -e "${colors[blue]}[+] Testing $url for LFI vulnerabilities...${colors[reset]}"
            sudo gau "$url" | sudo gf lfi | sudo uro | sed 's/=.*/=/' | sudo qsreplace "FUZZ" | sudo ffuf -u {} -w "$lfi_payloads" -c -mr "root:x:0:" -o "$output_dir/lfi_vul.txt"
        done < "$output_dir/alivesub.txt"
...
run_open_redirect_scan() {
    ...
    if [[ -f "$HOME/exnoscan/results/allurls.txt" && -s "$HOME/exnoscan/results/allurls.txt" ]]; then
        while IFS= read -r url; do
            echo -e "${colors[blue]}[+] Testing $url for Open Redirect vulnerabilities...${colors[reset]}"
            sudo ffuf -u "$url/FUZZ" -w "$HOME/exnoscan/or.txt" -mc 200 -c -v -mr "http" >> "$output_dir/open_redirect_vul.txt"
        done < "$output_dir/alivesub.txt"
...
run_xss_scan() {
    ...
    payload_file="$HOME/exnoscan/xss.txt"
    
    if [[ -f "$payload_file" ]]; then
        while IFS= read -r url; do
            echo -e "${colors[blue]}[+] Testing $url for XSS with DalFox and custom payloads...${colors[reset]}"
            sudo dalfox file "$output_dir/final.txt" --silence --custom-payload "$payload_file" --output "$output_dir/xss_vul.txt"
        done < "$output_dir/final.txt"
...
run_nuclei_scan() {
    ...
    if [[ -f "$output_dir/alivesub.txt" && -s "$output_dir/alivesub.txt" ]]; then
        sudo nuclei -l "$output_dir/alivesub.txt" -t cves,default-logins,exposures,vulnerabilities -severity low,medium,high,critical -o "$output_dir/multiple_vulnerabilities.txt"
...
cleanup_files() {
    ...
    sudo rm -f "$output_dir/subdomains.txt" "$output_dir/allurls.txt" "$output_dir/katana_endpoints.txt"
}
